...
pulumi config set HelpFullUi.Iac:sql-server-td-db-admin-id
pulumi config set HelpFullUi.Iac:sql-server-td-db-admin-password --secret

# Provide SMTP server's host, port, user, pass and default email sender.
pulumi config set HelpFullUi.Iac:default-email-from
pulumi config set HelpFullUi.Iac:email-server-host
pulumi config set HelpFullUi.Iac:email-server-port
pulumi config set HelpFullUi.Iac:email-server-userName
pulumi config set HelpFullUi.Iac:email-server-password --secret

# Create and provide an identity certificate password
pulumi config set HelpFullUi.Iac:identity-certificate-password --secret
```
